...
businesscsv=open("../dataset/yelp_training_set_business.csv","rb")
...
reviewcsv=open("../dataset/yelp_training_set_review.csv","rb")
...
for i in range(len(totalreviews)) :
	...
	if i%10 == 4 and d[1] in healthiddict :
		stars=int(totalreviews[i+2].split(':')[1])
		if stars > 3 :
			health_ratings.append("+")
		elif stars == 3 :
			health_ratings.append("0")
		else :
			health_ratings.append("-")
		c=totalreviews[i-2].split(':')
